...
const PreviewRoomModal: React.FC<Props> = ({
  ...
  const { mutate } = useJoinRoom(userInfo?.id);
 ...
 const handleJoin = () => {
  if (roomData?.privacy === "Private") {
    ...
    if (password && password.trim() !== "") {
      mutate({roomId:roomData?.id, userId:userInfo?.id , userName:userInfo?.name , password:password}); 
      onClose();
    } else {
    ...
    onJoin(roomData); 
    mutate({roomId:roomData?.id, userId:userInfo?.id , userName:userInfo?.name})
    onClose();
};
 }
